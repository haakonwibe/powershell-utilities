...
# base parameters
$containerUrl = "https://stautopilotscript.blob.core.windows.net"
$blobStorageResources = "resources"
$blobStorageHashes = "hashes"
$sasToken = "?sv=2019-12-12&ss=b&srt=o&sp=wx&se=2020-12-31T16:33:40Z&st=2020-08-30T07:33:40Z&spr=https&sig=8ykRjCT46Cn%2BW%2Ft1qpvkfyutBLGICDGLHApwQ8STdFA%3D"
...
if (-not (Test-Path $autoPilotScript)) {
    ...
    $webURL = "$containerUrl/$blobStorageResources/Get-WindowsAutoPilotInfo.ps1"
    $webclient.DownloadFile($webURL,"$outputPath\Get-WindowsAutoPilotInfo.ps1")
...
if (-not (Test-Path $azCopyExe)) {
    ...
    $webClient = New-Object System.Net.WebClient
    $webURL = "$containerUrl/$blobStorageResources/AzCopy.zip"
    $webclient.DownloadFile($webURL,"$outputPath\AzCopy.zip")
